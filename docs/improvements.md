...
2 An assignment whose right-hand side is a ternary still goes through hoistConditional's slot, where a *declaration* of the same shape rides acc across the merge (conditionalToAcc). `x = c ? 10 : 20` lowers to PUSH, a STORE per arm, LOAD, STORE; `u32 x = c ? 10 : 20` lowers to the arms plus one PUSH. Route the assignment RHS through the acc path — not "reuse the destination as the slot", which is what this item said before §8.7 stopped requiring a slot at all.
