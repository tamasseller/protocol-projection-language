...
| `fuzz/src/qemu-exec/` | the *emitted Thumb* on `qemu-system-arm` against the real `runtime/` | miscompilation — wrong answer, no crash |
...
$S/sweep.sh mog-jit/fuzz/seeds seeds   # standing regression check
```
...
npx ts-node --transpile-only mog-jit/fuzz/ts/oracle_server.ts /tmp/ppl-jit-oracle.sock

make -C mog-jit/fuzz/src/driver                     # fuzz_driver
PPL_FUZZ_CORPUS_OUT=/tmp/corpus ./fuzz_driver seeds # campaign + export

make -C mog-jit/fuzz/src/qemu-exec                  # exec_runner.elf
TS_NODE_PROJECT=mog-jit/fuzz/tsconfig.json \
  npx ts-node --transpile-only mog-jit/fuzz/ts/qemu-exec.ts mog-jit/fuzz/seeds

# triage
TS_NODE_PROJECT=mog-jit/fuzz/tsconfig.json npx ts-node --transpile-only \
  mog-jit/fuzz/ts/minimize-exec.ts [--hang] <file>   # ddmin over instructions, re-encoded
mog-jit/fuzz/dump_code.sh <program-file>         # translate + objdump: what was actually emitted
```
...
```sh
TS_NODE_PROJECT=mog-jit/fuzz/tsconfig.json npx ts-node --transpile-only mog-jit/fuzz/ts/make_seeds.ts
```
...
1. Read `mog-core/docs/isa-core.md` **and
   ...
   `mog-jit/docs/fuzzing-campaign.md` get fixed on the wrong side first.
2. Check whether the pre-existing validator and its test fixtures agree with
   ...
   forgotten.
3. Check whether real `src/codecs` output depends on the behavior —
...
- a seed in `make_seeds.ts` (so `qemu-exec.ts seeds` is a standing check), and
...
Then run everything: `cd mog-jit && make clean && make test`,
`make -C test/qemu stack-usage-check`, and `npm test` at the ppl repo
root.
...
`mog-jit/fuzz/README.md` documents the harness itself;
`mog-jit/docs/fuzzing-campaign.md` is the worked record of a full
