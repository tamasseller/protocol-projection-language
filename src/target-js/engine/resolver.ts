/**
 * target-js — The one generic TS-decl resolver
 *
 * A thin adapter over `src/core/projection.ts`'s `createResolver`, the same
 * primitive `codecs`'s `createCodecResolver` adapts. The artifact
...
import type { SemanticType, TypeGraph, TypeNode, TypePattern, MatchOf, ResolverRule } from "../../core/index"
...
export interface TsRule
{
     ...
     *  (`core`) only complains if something actually tries. See
 ...
 * emitted, so the full map is the actual deliverable — `core`'s
 ...
 * relative to `createCodecResolver` in `src/codecs/engine/resolver.ts`:
