...
// blocks.ts's own TABLE_PTR_REG/INDEX_REG — purely local to
// emitBrTableHelper's own few instructions (this file's header there has
// why r3/r1 specifically).
constexpr uint32_t TABLE_PTR_REG = 3;
constexpr uint32_t INDEX_REG = 1;

// isa-core.md §16 item 5: sized for CALL's own real-ABI sequence (this
// translator's single most expensive ordinary instruction) without
// pricing that cost into every cheaper ALU/LOAD/STORE/comparison
// instruction too — see blocks.ts's own header for the full reasoning.
...
uint32_t instrMaxBytes(const Instr &instr) {
    if(instr.op == Op::CALL) return CALL_MAX_BYTES;
    if(instr.op == Op::BR_TABLE && instr.imm > 2) return BR_TABLE_JUMP_OVERHEAD_BYTES + (uint32_t)(instr.imm + 1) * 2;
...
SpanResult maxSpanBytes(const uint8_t *bytes, uint32_t bytesLen, uint32_t from, uint32_t blockCount) {
    ...
    for(uint32_t remaining = blockCount; remaining > 0; remaining--) {
        for(;;) {
            ...
            pc = d.next;
            if(d.instr.op == Op::BLOCK_END || d.instr.op == Op::RETURN || d.instr.op == Op::TRAP) break;
        }
    }
    return {total, pc};
}
...
Frame openBrTable(Emitter &e, Window &window, uint32_t n, Cond condition, const uint8_t *bytes, uint32_t bytesLen, uint32_t pc) {
...
OpenedJump openBrTableJump(Emitter &e, Window &window, uint32_t n, AccState &accState) {
    ...
    uint32_t helperSite = e.placeholderBL();

    uint32_t base = e.pc(); // == lr, once the BL above actually executes
    for(uint32_t i = 0; i <= n; i++) e.emit(0); // n+1 slots, contiguous from base
...
bool resolveCaseClose(Emitter &e, Frame &frame, bool emitSkipToEnd) {
    ...
    if(frame.remaining > 0) return true; // stay on this frame — now translating the next case
    ...
    if(frame.table.present) e.patchLiteral(frame.table.endSlot, (uint16_t)(e.pc() - frame.table.base));
    return false;
}
...
bool closeBlockEnd(Emitter &e, Window &window, AccState &accState, Frame &frame,
    bool hasLoopExitCondition, Cond loopExitCondition,
...
uint32_t emitBrTableHelper(Emitter &e) {
    uint32_t start = e.pc();
    e.emit(ArmV6M::mov(ArmV6M::AnyReg(INDEX_REG), ArmV6M::AnyReg(ACC_REG))); // default: in range, index = selector
    e.emit(ArmV6M::cmp(R(ACC_REG), R(SCRATCH_REG)));
    uint32_t okSite = e.placeholderCondBranch(Cond::LS);
    e.emit(ArmV6M::mov(ArmV6M::AnyReg(INDEX_REG), ArmV6M::AnyReg(SCRATCH_REG))); // out of range: index = N — r0 untouched either way
    e.patchBranch(okSite, e.pc());
    e.emit(ArmV6M::lsls(R(INDEX_REG), R(INDEX_REG), ArmV6M::Imm<5>(1))); // halfword-indexed
    e.emit(ArmV6M::mov(ArmV6M::AnyReg(TABLE_PTR_REG), ArmV6M::AnyReg(LR))); // lr -> low reg, for LDRH's addressing
    ...
    // before the final BX (blocks.ts's own header has the full story).
    e.emit(ArmV6M::subs(R(TABLE_PTR_REG), ArmV6M::Imm<8>(1)));
    e.emit(ArmV6M::ldrh(R(SCRATCH_REG), R(TABLE_PTR_REG), R(INDEX_REG)));
    e.emit(ArmV6M::adds(R(TABLE_PTR_REG), R(TABLE_PTR_REG), R(SCRATCH_REG)));
    e.emit(ArmV6M::adds(R(TABLE_PTR_REG), ArmV6M::Imm<8>(1)));
    e.emit(ArmV6M::bx(ArmV6M::AnyReg(TABLE_PTR_REG)));
    return start;
}
