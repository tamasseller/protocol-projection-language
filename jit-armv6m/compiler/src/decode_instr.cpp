...
DecodedInstr decodeInstr(const uint8_t *bytes, uint32_t bytesLen, uint32_t offset)
{
    ...
    if(code <= 49)
    {
        ...
        if(mode == 0)
        {
            instr.combo = Combo::REG_ACC;
            instr.target = decodeLeb128(bytes, pos, next);
            return {instr, next};
        }
        if(mode == 1)
        {
            instr.combo = Combo::REG_REG;
            instr.target = decodeLeb128(bytes, pos, next);
            return {instr, next};
        }
        if(mode == 2)
        {
            instr.combo = Combo::PEEK_PEEK;
            return {instr, pos};
        }
        if(mode == 3)
        {
            instr.combo = Combo::POP_ACC;
            return {instr, pos};
        }
    ...
    case 99:
        instr.op = Op::CALL;
        instr.calleeIndex = decodeLeb128(bytes, pos, next);
        return {instr, next};
    case 100:
        instr.op = Op::RETURN;
        return {instr, pos};
