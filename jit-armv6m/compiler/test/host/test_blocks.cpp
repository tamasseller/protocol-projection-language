// jit-armv6m/compiler/test — blocks.h, ported from jit-armv6m/prototype/
// test/branch-range.test.ts, comparison-fusion.test.ts and blocks.ts's
// own unit-level correctness (openBrTable/closeBlockEnd's branch/table
// bookkeeping). End-to-end LOOP/BR_TABLE/comparison-as-value behavior is
// covered on real QEMU instead (test/qemu/fixtures.cpp) — this file is
// for the bookkeeping that's awkward to observe from the outside (a
// mis-patched branch is invisible in a passing *value* unless the test
// happens to exercise the exact path that would expose it).
#include "Test.h"
#include "blocks.h"
#include "emitter.h"
#include "window.h"
...
TEST(EmitGuardedBranchUsesLongFormWhenSpanExceedsRange)
{
    ...
    Instr body[16];
    for(int i = 0; i < 15; i++) body[i] = bare(Op::NOT);
...
{
    // The span being measured contains a whole LOOP construct (its own
    // cond + body sub-blocks) before reaching the closing terminator —
    // maxSpanBytes must recurse into it (rather than mis-measuring past
    // it) to still produce a correct bound for whatever guard is being
    // sized around this entire span.
    const Instr program[] = {
    ...
    // instrMaxBytes' own ORDINARY_MAX_BYTES(16) — nothing here is a CALL
    // or a BR_TABLE N>2 jump table.
    CHECK(r.bytes == 5 * 16);
    CHECK(r.nextPc == len);
}
...
TEST(MaxSpanBytesRecursesThroughANestedBrTable)
{
    ...
    // own separate nested-BR_TABLE branch in maxSpanBytes, distinct from
    // the nested-LOOP one above.
    const Instr program[] = {
        brTable(2),
...
TEST(MaxSpanBytesAccountsForANestedJumpTablesOwnOverhead)
{
    ...
    // genuine jump table, not an if/else fusion — instrMaxBytes' own
...
{
    // N == 1 (isa-core.md §7.1's bare "if", no else) — openBrTable's own
    // sibling branch to the N==2 if/else fusion the test right below this
    // one exercises: no case[1] to skip to, so the guard site self-links
    // as the sole entry in endFixupChain instead of populating
    // nextCaseFixup.
    ...
    Emitter e(buf, 8);
    Window window(0);
    Frame frame = openBrTable(e, window, 1, Cond::NE, bytes, len, 0);
    ...
    // Close the sole case — resolveCaseClose's own endFixupChain walk
    // (blocks.cpp) reads this self-linked site back via
    // Emitter::readBranchTarget, which must recognize it as the
    // *conditional*-branch shape (this construct's only guard never
    // goes through resolveCaseClose's own placeholderBranch() call,
    // unlike OpenBrTableIfElseFusion's own two-case chain link below,
    // which readBranchTarget sees as unconditional instead).
    AccState accState;
    e.emit(ArmV6M::mvns(ArmV6M::LoReg(ACC_REG), ArmV6M::LoReg(ACC_REG)));
    bool stillOpen = closeBlockEnd(e, window, accState, frame, false, Cond::EQ, bytes, len, 1);
    CHECK(!stillOpen);
...
TEST(OpenBrTableIfElseFusionPatchesNextCaseFixupToCase1Start)
{
    ...
    // single NOT too, then the whole construct closes (its own last
    // case). Verifies nextCaseFixup ends up pointing at case 1's real
    // start (right after case 0's own "skip to end" branch) and
    // endFixupChain resolves to the construct's own real end.
    ...
    Window window(0);
    AccState accState;
    Frame frame = openBrTable(e, window, 2, Cond::NE, bytes, len, 0);
    CHECK(frame.kind == FrameKind::Case);
    ...
    // case 0's own body: NOT (dest = ACC_REG, matching translate_proc.cpp's own dispatch).
    e.emit(ArmV6M::mvns(ArmV6M::LoReg(ACC_REG), ArmV6M::LoReg(ACC_REG)));
    bool stillOpen = closeBlockEnd(e, window, accState, frame, false, Cond::EQ, bytes, len, 1);
    ...
    uint16_t rawOff;
    // guardSite is the guard's own conditional branch (openBrTable's
    // emitGuardedBranch picked the short form for a span this small).
    CHECK(ArmV6M::getCondBranchOffset(buf[guardSite / 2], rawOff));
    int32_t delta = ArmV6M::signExtend(rawOff, 8) << 1;
    CHECK((uint32_t)((int32_t)guardSite + 4 + delta) == case1Start);

    // case 1's own body, then the construct's own last close.
    e.emit(ArmV6M::mvns(ArmV6M::LoReg(ACC_REG), ArmV6M::LoReg(ACC_REG)));
    int32_t skipSite = frame.endFixupChain; // case 0's own "skip to end" branch, still pending
    CHECK(skipSite >= 0);
    stillOpen = closeBlockEnd(e, window, accState, frame, false, Cond::EQ, bytes, len, 3);
    CHECK(!stillOpen);

    // case 0's own skip-to-end branch now resolves to the construct's
    // real end (right here).
...
TEST(CloseCaseViaTerminatorOnNonLastCaseKeepsFrameOpenAndResetsBookkeeping)
{
    ...
    Emitter e(buf, 8);
    Window window(0);
    window.tos = 3; // simulate a PUSH the case's own (never-emitted-here) body did
...
{
    // acc PENDING(imm), operand a register — §10.1's mirror-table
...
{
    // acc PENDING(imm) *and* the operand is a register — the mirrored-
    // condition shortcut's own precondition set, except fitsImm8(left.imm)
    // itself is false here, unlike
...
{
    // operand is a compile-time immediate but doesn't fit CMP's own imm8
    // form (§10.1's fallback, distinct from the imm8-fits case
...
TEST(MaterializeComparisonEmitsFiveInstructionIdiom)
{
    ...
    CHECK(e.halfwordCount() == 5); // CMP, Bcond, MOVS#1, B, MOVS#0 (design.md §10's own "4-5 instructions" estimate)
}
