#!/usr/bin/env bash
# Build and run the plugin self-test: a region around a loop of exactly known
# length, against a region around nothing. Fails loudly if the difference is
# not what the assembly says it must be.
#
...
PLUGIN_ARG="$(realpath ../plugin/bench_plugin.so)"
