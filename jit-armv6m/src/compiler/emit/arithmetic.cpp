...
static Effect addOrSubWithImm(AddSubRsubOp which, Assembler &e, uint32_t dest, uint32_t n, int32_t k)
{
    if(k == 0)
    {
        /*
...
Effect emitComparison(Assembler &a, Shape left, Op op, const Shape &operand)
{
    ...
    if(left.isImm() && !operand.isImm() && ArmV6M::fitsImm8(left.imm()))
    {
        ...
        return Effect::comparison(MIRRORED_CONDITION[idx]);
    }
    ...
    return Effect::comparison(condition);
}
...
Effect emitUnary(Assembler &e, Op op, uint32_t dest, uint32_t src)
{
    ...
    // The helper is an ordinary subroutine: r0-r3, r12 and the flags are gone.
    return Effect::clobber();
}
