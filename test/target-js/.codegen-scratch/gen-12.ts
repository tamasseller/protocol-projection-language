import { read, write, hasNext, cloneRd, cloneWr, seek, writeSeq, readSeq, readSeqView, writeSeqRaw, tagOf, signExtend, revBits, CodecTrap } from "ppl"
import type { Ctx } from "ppl"

export abstract class Result {}
export class Result_Ok extends Result {
  constructor(readonly value: number) { super(); }
}
export class Result_Err extends Result {
  constructor(readonly value: number) { super(); }
}

// proc 0: union
function encode_proc0(v0: Result, ctx: Ctx): void {
    write(ctx, 0, 1, tagOf(v0 instanceof Result_Ok ? "ok" : ("err"), ["ok","err"]));
    switch (tagOf(v0 instanceof Result_Ok ? "ok" : ("err"), ["ok","err"])) {
        case 0: {
            encode_proc1((v0 as Result_Ok).value, ctx);
            break
        }
        case 1: {
            encode_proc2((v0 as Result_Err).value, ctx);
            break
        }
        default: {
            break
        }
    }
    0;
    return;
}

// proc 1: integer
function encode_proc1(v0: number, ctx: Ctx): void {
    write(ctx, 0, 1, (v0) >>> 0);
    return;
}

// proc 2: integer
function encode_proc2(v0: number, ctx: Ctx): void {
    write(ctx, 0, 1, (v0) >>> 0);
    return;
}

// proc 0: union
function decode_proc0(ctx: Ctx): Result {
    let v0: any;
    switch (read(ctx, 0, 1)) {
        case 0: {
            const __tmp0 = decode_proc1(ctx);
            v0 = new Result_Ok(__tmp0);
            break
        }
        case 1: {
            const __tmp1 = decode_proc2(ctx);
            v0 = new Result_Err(__tmp1);
            break
        }
        default: {
            break
        }
    }
    0;
    return v0;
}

// proc 1: integer
function decode_proc1(ctx: Ctx): number {
    let v0: any;
    v0 = read(ctx, 0, 1);
    return v0;
}

// proc 2: integer
function decode_proc2(ctx: Ctx): number {
    let v0: any;
    v0 = signExtend(8, read(ctx, 0, 1));
    return v0;
}

export function encodeResult(value: Result): Uint8Array {
    const ctx: Ctx = { buffer: new Uint8Array(64), length: 0, iters: [{ pos: 0, capability: "write", overwriteOnly: false }] }
    encode_proc0(value, ctx)
    return ctx.buffer.subarray(0, ctx.length)
}

export function decodeResult(bytes: Uint8Array): Result {
    const ctx: Ctx = { buffer: bytes, length: bytes.length, iters: [{ pos: 0, capability: "read", overwriteOnly: false }] }
    return decode_proc0(ctx)
}