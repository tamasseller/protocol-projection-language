...
import {integer, list, named, struct, union, unit} from "../../src/core/metamodel"
import {AnyOfMatch, matchType, pAnyOf, pInteger, pList, pNamed, pStar, pStruct, pStructFields, pUnion, pUnit, StructFieldsMatch} from "../../src/core/matcher"
import {SemanticTypeKinds} from "../../src/core/metamodel"
...
////////////////////////////////////////////////////////////////////////////////////////////////
// pNamed — matching on a type's declared name (metamodel.ts's `named()`)
////////////////////////////////////////////////////////////////////////////////////////////////

test("pNamed: matches by name alone, regardless of shape", () => {
    const Ts = named("Timestamp", struct({secs: integer(0, 1)}))
    assert.notEqual(matchType(Ts, pNamed("Timestamp")), undefined)
})

test("pNamed: a different name doesn't match", () => {
    const Ts = named("Timestamp", struct({secs: integer(0, 1)}))
    assert.equal(matchType(Ts, pNamed("SomethingElse")), undefined)
})

test("pNamed: with inner, also requires the structural pattern to match", () => {
    const Ts = named("Timestamp", struct({secs: integer(0, 1)}))

    assert.notEqual(matchType(Ts, pNamed("Timestamp", pStruct({secs: pStar()}))), undefined)
    assert.equal(matchType(Ts, pNamed("Timestamp", pStruct({wrongField: pStar()}))), undefined)
})

test("pNamed: an unnamed type of the right shape doesn't match", () => {
    assert.equal(matchType(struct({secs: integer(0, 1)}), pNamed("Timestamp")), undefined)
})
