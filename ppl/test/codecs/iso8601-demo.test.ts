/**
 * src/codecs/test — a rule that overrides representation, not just adds one
 *
 ...
 * the way a C++ target's `std::optional<T>` rule would preempt the generic
 * union rule for a specific shape.
 *
 * `iso8601EncodeRule` matches the `Timestamp` shape already used in
 * `example`'s own schema (`{secs, nanos}`) and, instead of the
 ...
 * format), not a claim that date formatting belongs in `codecs`.
 */
import { describe, test } from "node:test"
import assert from "node:assert/strict"

import { struct, u8, u32, buildTypeGraph } from "../../src/core/index"
import { pStruct, pInteger } from "../../src/core/index"
