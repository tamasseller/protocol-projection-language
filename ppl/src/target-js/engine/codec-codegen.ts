/**
 * target-js — Compiled-source JS/TS codegen for a codec program: the
 ...
 * - `codecs`'s `resolveHandleTypes`/`resolveProcedureTypes`, for
 ...
 * (`src/codecs/engine/opcodes.ts`) — not just the binary family
 ...
 *
 * Unlike `codecs`'s own interpreter (`codec-extension.ts`), a
