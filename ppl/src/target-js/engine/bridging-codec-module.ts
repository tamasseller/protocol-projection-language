 ...
 * "what the local `Accessor` is keyed by." A codec image (`codecs`'s
 ...
 *
 * `core`'s `reconcile()`/`resolve()` compute the mapping (target- and
...
import type {SemanticType, TypeNode} from "../../core/index"
 ...
 *  `Correspondence`s instead (`resolveHandleCorrespondences`, `codecs`).
 ...
 * `core`'s `reconcile`/`resolve`), and a real, typed
